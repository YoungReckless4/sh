...
# Эта функция собирает скрытую информацию о функциях и записывает текущий номер версии сценария, время использования, версию системы, архитектуру ЦП, страну компьютера и имя функции, используемой пользователем. It does not involve any sensitive information, so don’t worry! Please believe me!
...
# The full text can be searched for the send_stats function call location. Он прозрачен и имеет открытый исходный код. If you have any concerns, you can refuse to use it.
...
UserLicenseAgreement() {
	...
	echo -e "----------------------"
	read -r -p "Согласны ли вы с вышеуказанными условиями? (да/нет):" user_input
...
install_add_docker() {
	echo -e "${gl_huang}Установка среды докера...${gl_bai}"
...
while true; do
	...
	case $sub_choice in
		...
		2)
			...
			read -e -p "Введите название изображения (разделяйте несколько названий изображений пробелами):" imagenames
			for name in $imagenames; do
				echo -e "${gl_huang}Обновление изображения:$name${gl_bai}"
				docker pull $name
			done
			;;
		3)
...
# Получите текущее значение параметра worker_processes
current_value=$(grep -E '^\s*worker_processes\s+[0-9]+;' "$CONFIG_FILE" | awk '{print $2}' | tr -d ';')

# Установите информацию о режиме на основе значения
...
ldnmp_Proxy() {
	...
	if [ -z "$reverseproxy" ]; then
		read -e -p "Пожалуйста, введите свой IP-адрес против поколения:" reverseproxy
	fi
	...
	while true; do
		...
		case $sub_choice in
			1)
				...
				read -e -p "Пожалуйста, введите ваше доменное имя:" yuming
				install_certbot
				docker run -it --rm -v /etc/letsencrypt/:/etc/letsencrypt certbot/certbot delete --cert-name "$yuming" -n 2>/dev/null
				install_ssltls
				certs_status
...
frps_panel() {
	...
	while true; do
		...
		echo "Официальный сайт: https://github.com/fatedier/frp/"
...
yt_menu_pro() {
	...
	while true; do
		...
		echo -e "Официальный адрес сайта: https://github.com/yt-dlp/yt-dlp."
		echo "-------------------------"
...
linux_update() {
	echo -e "${gl_huang}Выполняется обновление системы...${gl_bai}"
	if command -v dnf &>/dev/null; then
		dnf -y update
	elif command -v yum &>/dev/null; then
		yum -y update
...
linux_clean() {
	echo -e "${gl_huang}Идет очистка системы...${gl_bai}"
	if command -v dnf &>/dev/null; then
		rpm --rebuilddb
		dnf autoremove -y
		dnf clean all
...
correct_ssh_config() {
	...
	# Если найдено, для параметра PasswordAuthentication установлено значение «да».
	...
	# Если он найден, для параметра PubkeyAuthentication установлено значение «да».
	...
	# Устанавливает значение по умолчанию, если ни парольAuthentication, ни PubkeyAuthentication не совпадают.
...
add_sshkey() {
	...
	rm -rf /etc/ssh/sshd_config.d/* /etc/ssh/ssh_config.d/*
	restart_ssh
	echo -e "${gl_lv}Вход с закрытым ключом ROOT включен, вход с паролем ROOT отключен, и повторное подключение вступит в силу.${gl_bai}"
...
import_sshkey() {
	...
	rm -rf /etc/ssh/sshd_config.d/* /etc/ssh/ssh_config.d/*
	restart_ssh
	echo -e "${gl_lv}Открытый ключ был успешно импортирован, вход в систему с закрытым ключом ROOT включен, а вход с паролем ROOT закрыт. Повторное подключение вступит в силу.${gl_bai}"
...
echo "Установите свой ROOT-пароль"
passwd
sed -i 's/^\s*#\?\s*PermitRootLogin.*/PermitRootLogin yes/g' /etc/ssh/sshd_config;
sed -i 's/^\s*#\?\s*PasswordAuthentication.*/PasswordAuthentication yes/g' /etc/ssh/sshd_config;
rm -rf /etc/ssh/sshd_config.d/* /etc/ssh/ssh_config.d/*
restart_ssh
echo -e "${gl_lv}Настройка ROOT входа завершена!${gl_bai}"
...
dd_xitong() {
		...
		dd_xitong_4() {
		  ...
		  while true; do
			...
			echo -e "${gl_hui}Спасибо боссу leitbogioro и боссу bin456789 за поддержку сценариев!${gl_bai} "
			echo "------------------------"
			...
			case "$sys_choice" in
			  ...
			  2)
				send_stats "Переустановить дебиан 12"
				dd_xitong_1
...
clamav_scan() {
	...
	echo -e "${gl_lv}$@ Сканирование завершено, и отчет о вирусе сохраняется в${gl_huang}/home/docker/clamav/log/scan.log${gl_bai}"
...
create_backup() {
	...
	read -r -p "Введите каталог для резервного копирования (разделите несколько каталогов пробелами и нажмите Enter, чтобы использовать каталог по умолчанию):" input
...
linux_info() {
	...
	local runtime=$(cat /proc/uptime | awk -F. '{run_days=int($1 / 86400);run_hours=int(($1 % 86400) / 3600);run_minutes=int(($1 % 3600) / 60); if (run_days > 0) printf("%d天 ", run_days); if (run_hours > 0) printf("%d часов ", run_hours); printf("%d минут\n", run_minutes)}')
...
docker_ssh_migration() {
	...
	list_backups() {
		...
		echo -e "${BLUE}Текущий список резервных копий:${NC}"
	...
	backup_docker() {
		...
		echo -e "${YELLOW}Резервное копирование контейнеров Docker...${NC}"
		...
		[[ ${#TARGET_CONTAINERS[@]} -eq 0 ]] && { echo -e "${RED}Контейнер не найден${NC}"; return; }
		...
		for c in "${TARGET_CONTAINERS[@]}"; do
			echo -e "${GREEN}Резервный контейнер:$c${NC}"
			local inspect_file="${BACKUP_DIR}/${c}_inspect.json"
			docker inspect "$c" > "$inspect_file"

			if is_compose_container "$c"; then
				echo -e "${BLUE}обнаружен$cпредставляет собой контейнер для создания докеров${NC}"
				...
				if [[ -n "${PACKED_COMPOSE_PATHS[$project_dir]}" ]]; then
					echo -e "${YELLOW}Создать проект [$project_name] Резервная копия уже создана, пропустите повторную упаковку...${NC}"
					continue
				fi
				...
				if [ -f "$project_dir/docker-compose.yml" ]; then
					...
					echo -e "${GREEN}Создать проект [$project_name] В упаковке:${project_dir}${NC}"
				else
					echo -e "${RED}docker-compose.yml не найден, этот контейнер пропускается...${NC}"
				fi
			else
		...
		if [ -d "/home/docker" ]; then
			echo -e "${BLUE}Резервное копирование файлов в /home/docker...${NC}"
			find /home/docker -maxdepth 1 -type f | tar -czf "${BACKUP_DIR}/home_docker_files.tar.gz" -T -
			echo -e "${GREEN}Файлы в /home/docker были упакованы в:${BACKUP_DIR}/home_docker_files.tar.gz${NC}"
		fi

		chmod +x "$RESTORE_SCRIPT"
		echo -e "${GREEN}Резервное копирование завершено:${BACKUP_DIR}${NC}"
		echo -e "${GREEN}Доступные сценарии восстановления:${RESTORE_SCRIPT}${NC}"
	...
	restore_docker() {
		...
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}Каталог резервных копий не существует${NC}"; return; }

		echo -e "${BLUE}Начинаем операцию восстановления...${NC}"
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				if [[ "$running_count" -gt 0 ]]; then
					echo -e "${YELLOW}Создать проект [$project_name] Контейнеры уже запущены, пропустите восстановление...${NC}"
					continue
				fi
				...
				echo -e "${GREEN}Создать проект [$project_name] был извлечен в:$original_path${NC}"

				cd "$original_path" || return
				docker compose down || true
				docker compose up -d
				echo -e "${GREEN}Создать проект [$project_name] Восстановление завершено!${NC}"
			fi
		done

		# --------- Продолжаем восстанавливать нормальные контейнеры ---------
		echo -e "${BLUE}Проверьте и восстановите обычные контейнеры Docker...${NC}"
		local has_container=false
		for json in "$BACKUP_DIR"/*_inspect.json; do
			[[ ! -f "$json" ]] && continue
			has_container=true
			container=$(basename "$json" | sed 's/_inspect.json//')
			echo -e "${GREEN}Контейнер для обработки:$container${NC}"

			# Проверьте, существует ли контейнер и запущен ли он
			if docker ps --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${YELLOW}контейнер [$container] уже запущен, пропускаю восстановление...${NC}"
				continue
			fi

			IMAGE=$(jq -r '.[0].Config.Image' "$json")
			[[ -z "$IMAGE" || "$IMAGE" == "null" ]] && { echo -e "${RED}Информация о зеркале не найдена, пропустите:$container${NC}"; continue; }
			...
			if docker ps -a --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${YELLOW}контейнер [$container] существует, но не запущен, удалите старый контейнер...${NC}"
		...
		[[ "$has_container" == false ]] && echo -e "${YELLOW}Не найдена резервная информация для общих контейнеров.${NC}"

		# Восстановить файлы в /home/docker
		if [ -f "$BACKUP_DIR/home_docker_files.tar.gz" ]; then
			echo -e "${BLUE}Восстановление файлов в /home/docker...${NC}"
			mkdir -p /home/docker
			tar -xzf "$BACKUP_DIR/home_docker_files.tar.gz" -C /
			echo -e "${GREEN}Файлы в /home/docker восстановлены.${NC}"
		else
			echo -e "${YELLOW}Резервная копия файла в /home/docker не найдена, пропуск...${NC}"
		fi
	...
	migrate_docker() {
		...
		echo -e "${YELLOW}Перенос резервной копии...${NC}"
	...
	delete_backup() {
		...
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}Каталог резервных копий не существует${NC}"; return; }
		rm -rf "$BACKUP_DIR"
		echo -e "${GREEN}Удалена резервная копия:${BACKUP_DIR}${NC}"
	}
  ...
  while true; do
	...
	34)
	  ...
	  if [ -n "$filename" ]; then
		  ...
		  echo -e "${gl_huang}Разархивирование$filename ...${gl_bai}"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}31.  ${color31}Коллекция инструментов StirlingPDF${gl_kjlan}32.  ${color32}Drawio — бесплатное программное обеспечение для построения онлайн-графиков${gl_huang}★${gl_bai}"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}3.   ${gl_bai}Режим входа в систему с паролем ROOT${gl_kjlan}4.   ${gl_bai}Установите указанную версию Python"
	  ...
	  echo -e "${gl_kjlan}23.  ${gl_bai}Автоматическое отключение с ограничением тока${gl_kjlan}24.  ${gl_bai}Режим входа в систему с закрытым ключом ROOT"
	  ...
	  echo -e "${gl_kjlan}39.  ${gl_bai}Избранное команд${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}41.  ${gl_bai}доска объявлений${gl_kjlan}66.  ${gl_bai}Универсальная настройка системы${gl_huang}★${gl_bai}"
...
EOF
		  ...
		  13)
			  while true; do
				...
				while IFS=: read -r username _ userid groupid _ _ homedir shell; do
				  ...
				  echo "------------------------"
				  echo "1. Создайте обычный аккаунт 2. Создайте премиум-аккаунт"
				  echo "------------------------"
				  ...
				  case $sub_choice in
					  ...
					  2)
					   ...
					   # Предоставьте новому пользователю разрешения sudo
					   echo "$new_username ALL=(ALL:ALL) ALL" | tee -a /etc/sudoers
					  ...
					  4)
					   read -e -p "Пожалуйста, введите имя пользователя:" username
					   # Удалить разрешения пользователя sudo из файла sudoers
					   sed -i "/^$username\sALL=(ALL:ALL)\sALL/d" /etc/sudoers

						  ;;
					  5)
					   read -e -p "Пожалуйста, введите имя пользователя, которого хотите удалить:" username
					   # Удаление пользователей и их домашних каталогов
					   userdel -r "$username"
						  ;;
		  ...
		  echo "------------------------"
		  echo "1. Материковый Китай [по умолчанию] 2. Материковый Китай [Образовательная сеть] 3. Заморские регионы"
		  echo "------------------------"
		  echo "0. Вернуться в предыдущее меню"
		  echo "------------------------"
		  read -e -p "Введите свой выбор:" choice
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  ...
						  ;;
					  2)
						  read -e -p "Введите ключевые слова разобранного контента, который необходимо удалить:" delhost
		  ...
		  24)
			  ...
			  root_use
			  send_stats "Вход с закрытым ключом"
			  while true; do
				  clear
			  	  echo "Режим входа в систему с закрытым ключом ROOT"
			  	  echo "Видео-знакомство: https://www.bilibili.com/video/BV1Q4421X78n?t=209.4"
			  	  echo "------------------------------------------------"
			  	  echo "Будет сгенерирована пара ключей, более безопасный способ входа в систему через SSH."
				  echo "------------------------"
				  echo "1. Создайте новый ключ. 2. Импортируйте существующий ключ. 3. Просмотрите локальный ключ."
				  echo "------------------------"
				  echo "0. Вернуться в предыдущее меню"
				  echo "------------------------"
				  read -e -p "Пожалуйста, введите ваш выбор:" host_dns
		  ...
		  66)
			  ...
			  echo "1. Обновите систему до последней версии"
...
echo "------------------------"
echo -e "${gl_kjlan}Официальный сайт скрипта:${gl_bai}https://kejilion.sh            ${gl_kjlan}Адрес ГитХаба:${gl_bai}https://github.com/kejilion/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
echo -e "${gl_kjlan}------------------------${gl_bai}"
echo -e "${gl_kjlan}p.   ${gl_bai}Скрипт открытия сервера Eudemons Parlu"
echo -e "${gl_kjlan}------------------------${gl_bai}"
...
case $choice in
  ...
  p) send_stats "Скрипт открытия сервера Eudemons Parlu" ; cd ~
