...
disk_manager() {
	...
	while true; do
		...
		echo -e "${gl_huang}Эта функция находится на стадии внутреннего тестирования и не должна использоваться в производственной среде.${gl_bai}"
		echo "------------------------"
		list_partitions
		echo "------------------------"
...
EOF
		  ...
		  66)
			  ...
			  case "$choice" in
				[Yy])
				  clear
				  send_stats "Начинается комплексная настройка"
				  echo "------------------------------------------------"
				  linux_update
